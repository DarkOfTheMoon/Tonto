...
sub crash_file {
 my $errmsg = $_[0];
 my $filename = $filestack[$#filestack];
 my $fileline = $filelinenum{$filename};
 print STDERR "Error at line $fileline of $filename:";
 print STDERR $errmsg;
}

################################################################################
## -----------------------------------------------------------------------------
## Loop over the .foo file lines and generate the .fortran file lines
## -----------------------------------------------------------------------------
  ...
  my($parent_scope);
    ...
    if ($not_blank) {               # if not blank ...
      ...
      if ($newscopeunitfound) {
        ...
        elsif ($scopeunit eq 'interface' && $parentscope eq 'module') { &do_new_interface_scope($inp); }
        elsif ($scopeunit eq 'type' && $parentscope eq 'module') { &do_new_type_scope($inp); }
      ...
      elsif ($scopeunit eq 'program' || $scopeunit eq 'module') { &process_module($inp); }
      elsif ($scopeunit eq 'interface' && $parentscope eq 'module') { &process_module_interface($inp); }
...
sub analyse_rout_name {
    ...
    if (defined $2) {$name = $2} else {die "Error : no routine name, at $foohandle line $linenum.\n"; }
...
sub find_new_scoping_unit {
  ...
  if ($oldscope eq 'function' || $oldscope eq 'subroutine' || $oldscope eq 'program') {
    if ($_[0] =~ m'do'o && $_[0] =~ m'(?:^|:) *do *(?: [a-zA-Z]|$|[\#!;])'o) { # do
        ...
        $newscope = 'select';
    } elsif ($_[0] =~ m'forall'o && $_[0] =~ m'^ *forall *[(]'o) { # forall
        $newscope = 'forall';
...
sub do_empty_end {
  ...
  if (($scopeunit eq 'subroutine' || $scopeunit eq 'function') && $#scope <= 3) {
      ...
      if (defined $routine_attributes{$name}{'inherited'}) {
          $getfile = lc($routine_attributes{$name}{'parent_module'}). ".get";
          if ($foodir ne "") {
...
sub html_process_foo_line {
  ...
  if (! $being_inherited) { # We only want the original foo, not inserted stuff.
    if ($not_blank) {
      if ($newscopeunitfound) {
        if ($parentscope eq 'module' && ! $html_done_use_list) { &html_put_use_list; }
        ...
        elsif ($lastfoundscope eq 'interface' && $parentscope eq 'module') { &html_start_module_interface; }
        elsif ($lastfoundscope eq 'type' && $parentscope eq 'module') { &html_start_module_type; }
      ...
      elsif ($scopeunit eq 'interface' && $parentscope eq 'module') { &html_process_module_interface; }
...
sub fortran_process_foo_line {
  ...
  if ($not_blank) {
    ...
    if ($newscopeunitfound) {
      ...
      if ($lastfoundscope eq 'interface' && $parentscope eq 'module') { &fortran_start_module_interface; }
...
# named-end?
    elsif ($scopeunit eq 'interface' && $parentscope eq 'module') { &fortran_process_module_interface; }
    elsif ($scopeunit eq 'interface' && $parentscope =~ '(subroutine)|(function)') { &fortran_process_routine_interface; }
# should the following be elseif's?
    if ($scopeunit eq 'program') { &fortran_process_program; }
...
sub fortran_add_stack_macro {
  ...
  if ($this_is_first_active_line && ! defined $routine_attributes{$name}{'pure'}) {
      $fortran_out =~ s/^(\s*)/$1STACK(\"$module_name:$routine_attributes{$name}{'real_name'}\")\n$1/;
      undef %arglist;
      $arglist{"argument name"} = "argument index";
  }
...
sub fortran_process_return {
  ...
  if ($fortran_out =~ '.return') {
    if ($fortran_out =~ '[)] *return *(?:!|$)' ) {
      if (defined $routine_attributes{$name}{'pure'})     { }
      elsif (defined $routine_attributes{$name}{'leaky'}) { $fortran_out =~ s/[)] *return */) then; UNSTACK return; end if/o; }
      else                                         { $fortran_out =~ s/[)] *return */) then; CHECK return; end if/o; }
    }
    if ($fortran_out =~ '(?:^|;) *return *' ) {
      if (defined $routine_attributes{$name}{'pure'})     { }
      elsif (defined $routine_attributes{$name}{'leaky'}) { $fortran_out =~ s/return/UNSTACK return/o; }
...
################################################################################
# Add macros include, SELF_TYPE, and ELEMENT_TYPE file.
...
sub fortran_misc_changes {
  ...
  elsif ($fortran_out =~ ' use ' && $fortran_out =~ s/^[ ]*use[ ]+([A-Z]\w*)/   use $1_MODULE/s) { }
...
sub fortran_do_empty_end {
  ...
  if (($oldscope eq 'subroutine' || $oldscope eq 'function') && $#scope < 3) {
    if ($being_inherited) {
      ...
      if (defined $routine_attributes{$name}{'pure'})     { $fortran_out =~ s/end */end $oldscope/; }
      elsif (defined $routine_attributes{$name}{'leaky'}) { $fortran_out =~ s/(\s*)end */$1  UNSTACK\n$1end $oldscope/; }
      else                                         { $fortran_out =~ s/(\s*)end */$1  CHECK\n$1end $oldscope/; }
    }
  } else {
