...
# 0 or default: Part line (from within a keyword)
# 1: Full line (from Parse function)
##############################################################################
my $running = 0;
my $currentproc = 0;
sub RunProcessors
{
	...
	if($ProcessorTypes[$i] == 0 ||
	   ($calledfrom == 0 && $ProcessorTypes[$i] == 2) ||
	   ($calledfrom == 1 && $ProcessorTypes[$i] == 1)) {
	    $string = $Processors[$i]->($string);
	}
	if(!$recursing) { $currentproc = $i; }
    }

    if(!$recursing) { $running = 0; }
...
sub PrintProcessors
{
	...
	Debug($processor." type ".$ProcessorTypes[$i], 3);
	$i++;
    }
}
...
sub AddProcessor
{
	...
	@ProcessorTypes = reverse(@ProcessorTypes);
    }
    push(@Processors, $function);
    push(@ProcessorTypes, $type);
    if($first) {
	@Processors = reverse(@Processors);
	@ProcessorTypes = reverse(@ProcessorTypes);
    }
...
sub RemoveProcessor
{
	...
	$ProcessorTypes[$i] = $ProcessorTypes[$i+1];
    }
    pop(@Processors);
    pop(@ProcessorTypes);
...
sub FindDefineArgs
{
    ...
    # check if last macro ends in ... (means any number of args in macro)
    if($#Argnames >= 0 && $Argnames[$#Argnames] =~ /\.\.\.\Z/o &&
       $#Argvals >= $#Argnames) {
	my $lastarg = pop(@Argvals); # get lastarg
	# append all extra args into final arg
	my @Extras;
	while($#Argvals >= $#Argnames) {
	    push(@Extras, pop(@Argvals));
	}
	my $extra = ($#Extras>=0?(pop(@Extras)):"");
	while($#Extras >= 0) { $extra = $extra.", ".pop(@Extras); }
	push(@Argvals, $extra);   # return concatanated args
	push(@Argvals, $lastarg); # replace lastarg
    }
    # check the right number of args have been passed, should be all args 
    # present plus string at end of args
    elsif($#Argvals != ($#Argnames+1)) {
	# show warning if wrong args (unless macro should have zero args and
	# 1 arg provided which is blank space
	if(!($#Argnames == -1 && $#Argvals == 1 && $Argvals[0] =~ /\A\s*\Z/)) {
...
sub ReplaceDefineArgs
{
	...
	if($#Argnames >= 0 && $Argnames[$#Argnames] =~ s/\.\.\.\Z//o) {
		...
		if($Argvals[$#Argnames] eq "") {
		    $string =~ s/\,\s*\#\#$Argnames[$#Argnames]//g;
		}
		else {
		    $string =~ s/\#\#$Argnames[$#Argnames]/$Argnames[$#Argnames]/g;
		}
	    }
	}
...
{ 	my %RealDefines = %Defines;
	...
	if(!$Argvals[$#Argnames+1]) { $Argvals[$#Argnames+1] = ""; }	
	$tail = $Argvals[$#Argnames+1];
  ...
  # }
	...
	if($thisline && $thisline =~ /^$keywordchar/) {
		...
		# check for 'endif' style keyword
		    ...
		    if($Keywords{$keyword}->($input)) { $iflevel--; }
		}
		# run all other keywords
		elsif($Writing[$iflevel]) { $Keywords{$keyword}->($input); }
	...
	elsif(!$found && $preserveblank) { Filepp::Output("\n"); }
...
{ my $func;  foreach $func (@OpenOutputFuncs) { $func->(); } }
...
# run any close functions
{ my $func;  foreach $func (@CloseOutputFuncs) { $func->(); } }
# close output file
close(OUTPUT);
